    ...
    for (int y = 0; y < image_height_; y++) {
        for (int x = 0; x < image_width_; x++) {
            ...
            if (y >= 1 && y < image_height_ - 1 && x >= 1 && x < image_width_ - 1) {
                pixel.crossing = Pixel::StatusCrossing::Inside; // inside (0)
            }
            ...
            int unknown_factor = pooling_image_[0][x + y * image_width_].known
                + pooling_image_[0][left + top * image_width_].known
                + pooling_image_[0][x + top * image_width_].known
                + pooling_image_[0][right + top * image_width_].known
                + pooling_image_[0][right + y * image_width_].known
                + pooling_image_[0][right + bottom * image_width_].known
                + pooling_image_[0][x + bottom * image_width_].known
                + pooling_image_[0][left + bottom * image_width_].known
                + pooling_image_[0][left + y * image_width_].known;
            if (unknown_factor == 18) // All pixel are unknown -> 9 * unknown (2) = 18
                pixel.known = Pixel::StatusKnown::Unknown;        // unknown (2)
            else if (unknown_factor > 0)                          // Some pixel are unknown
                pixel.known = Pixel::StatusKnown::
...
#pragma omp parallel for
        for (int y = 0; y < image_height_; y++) {
            for (int x = 0; x < image_width_; x++) {
                ...
                if (x - s_half < 0) {
                    left = 0;
                    pixel.crossing = Pixel::StatusCrossing::Crossing; // crossing (1)
                }
                else {
                    left = x - s_half;
                }
...
*/
                ...
                int unknown_factor = pooling_image_[l - 1][left + top * image_width_].known
                    + pooling_image_[l - 1][right + top * image_width_].known
                    + pooling_image_[l - 1][left + bottom * image_width_].known
                    + pooling_image_[l - 1][right + bottom * image_width_].known;
...
DensePoolingImage<PinholeCamera>::poolBoundingBox(int u_min, int u_max, int v_min, int v_max) const
{
    ...
    for (int v = v_min + s; true; v += step) {
        ...
        for (int u = u_min + s; true; u += step) {
            ...
            if (pixel.known == Pixel::StatusKnown::Unknown) {
                count_unknown_pixel++;
                count_partly_known_pixel++;
            }
            else if (pixel.known == Pixel::StatusKnown::PartKnown) {
                count_partly_known_pixel++;
            }
    ...
    if (count_pixel == count_unknown_pixel) {
        pixel_batch.known = Pixel::StatusKnown::Unknown;
    }
    else if (count_partly_known_pixel > 0) {
        pixel_batch.known = Pixel::StatusKnown::PartKnown;
    }
                                                    ...
                                                    const Eigen::Vector2i& bb_max) const
{
    ...
    if (!u_in || !v_in) {
        pix.crossing = Pixel::StatusCrossing::Crossing;

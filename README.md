...
#### 1. Prepare dataset (TUM-RGBD format)

We use an example dataset extracted from a ROS2 bag recorded in a customized Gazebo ship simulator with synthetic iron corrosion texture maps.

* ROS2 bag: [https://inside.autoassess.eu/apps/files/files/381745?dir=/AUTOASSESS/Datasets/2025-11-Simulator/ship_CH](https://inside.autoassess.eu/apps/files/files/381745?dir=/AUTOASSESS/Datasets/2025-11-Simulator/ship_CH)
* Simulator: [https://inside.autoassess.eu/f/400274](https://inside.autoassess.eu/f/400274)
* Example dataset: [https://inside.autoassess.eu/apps/files/files/416995?dir=/AUTOASSESS/Work%20Packages/WP6/Task%206.3/ship_CH_data](https://inside.autoassess.eu/apps/files/files/416995?dir=/AUTOASSESS/Work%20Packages/WP6/Task%206.3/ship_CH_data)

The dataset follows the [TUM-RGBD format](https://cvg.cit.tum.de/data/datasets/rgbd-dataset/file_formats) and should be organized as:

```
├── groundtruth.txt            # Ground-truth trajectory (timestamp tx ty tz qx qy qz qw)
├── intrinsics.txt             # Camera intrinsics (fx, fy, cx, cy)
├── rgb                        # Timestamped RGB images
│   ├── 1762179077.257259.png
│   ├── 1762179077.335727.png
│   └── ...
├── depth                      # Timestamped depth images (scale factor: 5000.0)
│   ├── 1762179077.220248.png
│   ├── 1762179077.276930.png
│   └── ...
├── mask                       # Instance (defect) segmentation masks (aligned with RGB)
│   ├── 1762179077.257259.png
│   ├── 1762179077.335727.png
│   └── ...
├── rgb.txt                    # RGB file list (timestamp, relative path to each RGB image)
├── depth.txt                  # Depth file list (timestamp, relative path to each depth image)
└── mask.txt                   # Mask file list (timestamp, relative path to each mask image)
```

**Note:**
The `mask/` directory is generated by running [DTU's defect segmentation network](https://github.com/AUTOASSESS/dsn-pytorch-inference) on each RGB image.

#### 2. Configure parameters

Update `config/ship_CH_test.yaml`:

* `sensor[width], sensor[height]`: resolution of RGB images
* `sensor[fx], sensor[fy], sensor[cx], sensor[cy]`: values from `intrinsics.txt`
* `sensor[T_BS]`: extrinsics between `groundtruth.txt` poses and camera frame (if applicable)
* `reader[sequence_path]`: path to the dataset
* `reader[ground_truth_file]`: path to `groundtruth.txt`
* `app[mesh_path]`: output path for the final fused mesh

#### 3. Run semantic fusion

```
./build/release/app/supereight_tsdfcolid_single_pinholecamera config/ship_CH_test.yaml
```

A visualization window will open to display the fusion process in real time. The reconstructed and fused mesh will be saved to the path specified in `app[mesh_path]`.

#### 4. Output visualization

An example output mesh can be downloaded [here](https://inside.autoassess.eu/f/381743).

* **Face colors**: represent defect types (randomly assigned per class, as defined by DTU's segmentation network)
* **Vertex colors**: represent the original RGB appearance of the scene
...
An example output can be downloaded [here](https://inside.autoassess.eu/apps/files/files/2553?dir=/AUTOASSESS/Work%20Packages/WP6/Task%206.4).
